...
**⚠️ `ACT_NUM_ACTUACIO` ha desaparegut de l'esquema en viu (verificat 2026-08-09)**: el camp ja no apareix a `fields[]` de `?f=json` ni es retorna encara que es demani explícitament a `outFields` — es descarta en silenci, igual que ja passava amb `EditDate` (§ anterior sobre `arcgis.py`). Sense clau, `_dedupe_features` descartava totes les files (`if not act_num: continue`), de manera que `fetch_incidents` retornava sempre una llista buida sense cap error HTTP: `binary_sensor.servei_connectat` seguia `on` i `estat_de_la_darrera_actualitzacio` seguia `success` mentre 0 incendis es feien seguir. `models.incident_key` (`models.py`) ara fa fallback a `MUNICIPI_SIG`/`MUNICIPI_DPX` + `ACT_DAT_INICI` (estable per incendi, ja que cap dels dos canvia mentre dura l'actuació) i, en últim cas, a `OBJECTID`/`GlobalID`. Si `ACT_NUM_ACTUACIO` torna a aparèixer, `incident_key` torna a fer-lo servir automàticament sense cap canvi de codi.
